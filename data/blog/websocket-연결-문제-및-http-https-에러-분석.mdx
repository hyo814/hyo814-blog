...
웹소켓 연결 시 서버에서 발생하는 에러를 분석하다 보면, 일정하지 않은 상황에서도 계속 웹소켓 연결을 시도하는 경우를 마주합니다. 이 때문에 서버를 내리고 재시작한 직후 연결 오류가 발생하기도 합니다.
...
프론트엔드에서 HTTPS로 요청을 보내는데 서버가 HTTP만 지원하면 다음과 같은 오류가 납니다:
...
서버가 클라이언트의 요청을 처리하는 과정에서 발생한 오류입니다. 원인은 HTTP 요청의 메서드 이름이 유효하지 않다는 점입니다.
...
- 로그에 보이는 16진수 값들(예: `0x16, 0x03, 0x01, ...`)은 SSL/TLS 핸드셰이크 데이터를 HTTP 요청으로 잘못 처리한 결과입니다.
- 클라이언트가 `wss://` 프로토콜로 요청을 보내면 서버는 SSL 연결을 처리할 수 있어야 합니다. 하지만 HTTP만 지원하는 서버에서는 이 데이터를 파싱하지 못해 오류가 납니다.

### 해결 방안

1. **SSL 인증서 설정**:

   - 서버에 SSL 인증서를 설정해 HTTPS와 `wss://` 요청을 지원.
...
2. **클라이언트 요청 수정**:
   - 서버가 HTTP만 지원하면 `ws://`로 요청.
   - 예:
     ```jsx
     const socket = new WebSocket('ws://example.com/socket')
     ```
3. **프록시 설정**:
   - Nginx나 Apache 같은 프록시 서버로 HTTPS 요청을 받은 뒤 백엔드로 전달.

### 요약

- 클라이언트 요청 방식(`wss://` vs `ws://`)과 서버 지원 프로토콜(HTTP vs HTTPS)을 맞추는 것이 중요합니다.
- SSL 인증서를 설정하거나 프록시 서버를 두는 방식으로 해결합니다.

---

### 결론

이 문제는 WebSocket 프로토콜, SSL/TLS, HTTP 요청 처리 방식의 불일치에서 생깁니다. 인증서를 알맞게 설정하고 프로토콜을 맞추면 풀립니다. 문제가 발생했을 때는 로그로 원인을 정확히 짚고, 단계적으로 해결 방안을 적용하는 것이 좋습니다.
