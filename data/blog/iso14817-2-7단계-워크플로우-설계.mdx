...
## 7. 전이 표를 다시 세어보니 — 반려할 수 없는 단계가 둘 있다

4절의 전이 9개를 반려 가능 여부로 다시 정리하면 이렇게 됩니다.

| 상태 | 담당 | 승인 | 반려 |
|---|---|:-:|:-:|
| SUBMIT | 관리자 | → PRE_VERIFY | → RECORD |
| PRE_VERIFY | 위원회 | → VERIFY | → RECORD |
| **VERIFY** | **관리자** | → PRE_CERTIFY | **없음** |
| PRE_CERTIFY | 위원회 | → CERTIFY | → VERIFY |
| **CERTIFY** | **관리자** | → DEPLOY | **없음** |

**관리자가 담당하는 두 단계(검증·인증)에는 반려 경로가 없습니다.** 2절 표에서 이 둘을 *"관리자 2차 검토 대기열"*, *"관리자 최종 승인 대기열"* 이라고 적어놨는데, **검토인데 통과만 시킬 수 있어요.** 관리자가 문제를 발견해도 되돌릴 수단이 없습니다.

반려 3개가 전부 *"위원회가 보기 전"* 또는 *"위원회가 보고 나서"* 에 걸려 있고, 관리자의 본 단계는 사실상 **통과 도장**만 찍는 구조예요. 3절에서 관리자를 *"위원회 앞에 한 번 걸러주는 게이트"* 라고 설명한 것과도 어긋납니다. 게이트라면 막을 수 있어야 하니까요.

의도한 설계일 수도 있습니다 — 사전 단계에서 이미 걸렀으니 본 단계는 형식적 확인이라고 보는 해석이요. 그렇다면 **2절의 "검토 대기열"이라는 표현을 고쳐야** 하고, 아니라면 전이 두 개를 추가해야 합니다. 어느 쪽이든 **표를 세어보기 전에는 몰랐다**는 게 문제입니다.

이 글이 *"반려는 일괄적으로 처음으로 되돌리지 않는다"* 를 교훈으로 내세우면서, 정작 **반려가 아예 없는 단계**는 짚지 않았어요. 되돌림 깊이를 설계할 때 *"되돌릴 수 있는가"* 부터 표로 확인했어야 했습니다.

---

## 8. 그 밖에 남은 것

**① `STATE_FILTERS`에 제출자가 없습니다.**
...
3절에서 *"이 필터가 있으면 역할만 주면 끝이고, 상태 추가 시 코드가 퍼지지 않는다"* 고 했는데, **세 역할 중 하나가 이 구조 밖에 있습니다.** 제출자의 목록은 다른 방식으로 만들어지고 있다는 뜻이고, 그러면 상태를 추가할 때 결국 두 군데를 봐야 해요. 필터의 장점이 절반만 실현된 상태입니다.

**② 5단계 버전이 전이 목록을 복제합니다.**

`ISO14817SimpleFactory`는 `create()`만 오버라이드하는데, 그 안에 `workflow_stages` 리스트가 **통째로 다시 쓰여 있습니다.** 7단계와 5단계가 공유하는 전이(RECORD→SUBMIT, SUBMIT→PRE_VERIFY, SUBMIT→RECORD, PRE_VERIFY→VERIFY)가 두 곳에 존재해요.

*"선언만 바꿔서 만들 수 있다"* 는 7절의 주장은 맞지만, **선언이 복제되는 방식**입니다. 전이를 상수로 빼두고 팩토리는 *어떤 것을 포함할지* 만 고르게 했으면 진짜 선언형이 됐을 거예요. 상속으로 푼 것도 다시 보면 애매합니다 — 두 팩토리의 관계는 *부모-자식*이 아니라 **같은 부품으로 만든 다른 조합**에 가깝습니다.

**③ 표준 준수를 확인할 장치가 없습니다.**

ISO 14817-2의 상태를 한국어 운영 용어로 매핑했는데, **원문 용어와의 대응표가 코드에 없습니다.** 나중에 누가 상태를 하나 추가하거나 이름을 바꿔도 "이게 표준을 벗어났는지" 알 방법이 없어요.

상태 상수 옆에 표준 원문 용어를 주석으로 달아두거나, 매핑을 별도 상수로 두는 것만으로도 충분했을 겁니다. 표준을 기준선으로 삼은 것이 이 설계의 핵심 근거인데, **그 기준선이 코드에 남아 있지 않으면 시간이 지나며 조용히 멀어집니다.**
