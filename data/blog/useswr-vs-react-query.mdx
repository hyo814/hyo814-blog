...
const InvalidateButton = () => {
  ...
  return <button onClick={handleInvalidate}>Invalidate Todos</button>
}
```
...
const ResetButton = () => {
  ...
  return <button onClick={handleReset}>Reset Todos</button>
}
```
...
const SetDataButton = () => {
  ...
  return <button onClick={handleSetData}>Add New Todo</button>
}
```
