...
## 9. 다시 보니 — 권한 글에 남아 있던 구멍

개고하면서 위 코드를 다시 읽었고, "권한 중심으로 재설계하라"고 쓴 글이 정작 놓친 것들이 보입니다.

- **`{{ post.content | safe }}` — 이 글에서 제일 위험한 줄입니다.** 사용자가 쓴 문의 내용을 이스케이프 없이 렌더합니다. 문의 본문에 `<script>`를 넣으면 **그 글을 열람하는 관리자의 브라우저에서 실행**돼요. 비공개 게시판이라 공격 표면이 좁아 보이지만, "사용자 입력 → 관리자가 열람"은 오히려 stored XSS의 고전 경로입니다. 리치 텍스트가 필요했다면 `| safe`가 아니라 서버 측 sanitize(bleach 등)를 거친 뒤에만 안전 표시를 해야 합니다. 권한 매트릭스를 그려놓고 콘텐츠 이스케이프를 뚫어놓은 셈이라, 접근 제어와 입력 처리는 **별개의 방어선**이라는 걸 보여주는 사례예요.
- **3절의 원칙을 3절의 코드가 안 지킵니다.** "뷰에서 체크하기 전에 QuerySet부터 필터링"이라고 써놓고, `InquiryDetailView`는 QuerySet이 아니라 `get_object`에서 **뷰 레벨 체크**를 합니다. `get_queryset`을 목록 뷰와 같은 필터로 오버라이드하면 타인 글은 애초에 조회가 안 돼 404가 **공짜로** 나오고, 체크 코드 자체가 사라져요. 원칙이 맞았는데 코드가 원칙을 따라가지 못한 자기모순입니다. **그리고 넉 달 뒤 실제 시스템에 붙일 때도 이 결론을 안 지키고 또 403으로 냈습니다** — 경위는 [게시판 종류를 코드가 아니라 데이터로 뒀다](/blog/게시판-종류를-코드가-아니라-데이터로-뒀다)에 적었습니다.
- **`CommentCreateView`는 없는 `post_id`에 500을 냅니다.** `form_valid`에서 `self.kwargs['post_id']`를 검증 없이 FK에 넣으니, 삭제된 문의에 답변을 제출하면 IntegrityError가 서버 에러로 터집니다. `get_object_or_404(Post, pk=...)`로 먼저 존재를 확인하는 게 맞습니다.
