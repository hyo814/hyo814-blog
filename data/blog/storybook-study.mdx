...
디자인 시스템은 브랜드 정체성을 시각·기능 양면으로 드러내는 데 필요한 규칙, 가이드라인, 컴포넌트, 툴의 묶음입니다. 개발자와 디자이너가 함께 일하기 쉬워지고, 제품이 일관된 사용자 경험(UX)을 갖추도록 돕습니다.
...
디자인 시스템은 제품 전반에 같은 스타일과 동작을 유지해 사용자에게 일관된 경험을 줍니다.

### 3.2 개발 및 디자인 효율성 향상

재사용 가능한 컴포넌트로 개발 시간과 디자인 리소스를 아낄 수 있습니다.
...
새 기능이나 페이지를 추가할 때 기존 디자인 시스템을 쓰면 확장이 쉽고 유지보수도 간단합니다.
...
- `Label` 컴포넌트는 텍스트를 표시하는 단순한 UI 요소로, 다양한 스타일과 상태를 갖습니다.
- Storybook으로 `Label` 컴포넌트의 다양한 사용 사례를 정의하고, 팀원들과 쉽게 공유하도록 설정합니다.

---

### 2. **Metadata 설정**

스토리는 Storybook의 핵심 구조로, 컴포넌트를 렌더링하는 데 필요한 정보를 담습니다. `metadata`는 스토리의 기본 설정을 관리하며, 아래 속성을 지정합니다.
...
`TextField` 컴포넌트는 입력 필드를 제공하는 기본 UI 요소로, 여러 상황에 대응하려고 작은 단위의 Atom 컴포넌트를 안에 둡니다.

그중 `ErrorMessage` 컴포넌트는 사용자 입력이 유효하지 않을 때 알맞은 피드백을 주는 역할을 합니다.
...
`TextField`는 입력 필드에 부가 기능과 상태를 더하려고 **Atomic 디자인 패턴**으로 하위 컴포넌트를 조합합니다.

작은 컴포넌트(Atoms)들을 모아 복잡한 UI를 구성할 수 있습니다.
...
`NavigationBar`는 애플리케이션 페이지 사이를 오가는 핵심 UI 컴포넌트입니다.

주로 화면 위나 아래에 자리하며, 사용자 경험을 끌어올리려고 스타일과 아이콘을 함께 둡니다.
...
`PrimaryButton`은 애플리케이션에서 주로 쓰이는 핵심 액션 버튼입니다.

사용자 상호작용에서 중요한 역할을 하며, 활성·비활성·로딩 등 여러 상태를 갖습니다.
...
`TagList` 컴포넌트에 **Generic Type**을 적용하면 데이터 타입의 유연성을 확보할 수 있습니다.

또 **Event Bubbling**으로 개별 태그의 클릭이나 삭제 이벤트를 상위 컴포넌트에서 처리하도록 설계할 수 있습니다.
