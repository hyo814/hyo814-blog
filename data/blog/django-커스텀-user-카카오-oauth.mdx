...
## 8. 다시 보니 — 이 글 코드에 남아 있는 구멍

개고하면서 위 코드를 다시 읽었고, "직접 구현"의 대가가 정확히 어디서 청구되는지 보입니다.

- **본문과 코드가 서로 모순입니다.** 6.1에서 "카카오 유저는 `set_unusable_password()`가 걸린 상태"라고 썼는데, 5절의 `get_or_create`는 **그걸 호출하지 않습니다.** defaults로 만든 유저의 password는 빈 문자열로 남아요. Django는 빈 해시 인증을 통과시키지 않아서 사고로는 안 이어졌지만, "안 터진 것"과 "안 터지게 해둔 것"은 다릅니다. 생성 분기에서 `user.set_unusable_password()`를 명시적으로 호출해야 글과 코드가 일치합니다.
- **`state` 파라미터가 없습니다.** OAuth 2.0의 `state`는 콜백이 내가 시작한 플로우의 응답인지 확인하는 CSRF 방어입니다. 없으면 공격자가 자기 카카오 계정의 code로 피해자 브라우저에서 콜백을 열어 **피해자를 공격자 계정에 로그인시키는** 로그인 CSRF가 가능합니다. 인가 요청 때 세션에 난수를 넣고 콜백에서 대조하는 코드가 몇 줄이면 되는데, 라이브러리였다면 기본으로 해줬을 일입니다 — 7절의 "직접 구현" 선택이 지불한 비용이 바로 이런 항목이에요.
- **에러 메시지가 계정 존재를 노출합니다.** "카카오 로그인으로 가입된 계정입니다"는 UX로는 친절하지만, **그 username의 계정이 존재한다는 사실**을 확인시켜 줍니다(user enumeration). 커뮤니티 서비스 수준에서는 감수할 만한 트레이드오프라고 판단하지만, 트레이드오프인 걸 모르고 쓴 것과 알고 쓴 것은 다르니 적어둡니다.
- 사소하게는, 5절에서 "동의 거부를 500으로 튕기지 말자"고 해놓고 **토큰 교환 실패는 `raise_for_status()`로 그대로 500**이 됩니다. 뒤로가기로 만료된 code가 재사용되는 흔한 경로라, 여기도 로그인 페이지로 돌려보내는 처리가 맞습니다.
